 ...
 */
export function detectLLMFromUserAgent(
 ...
 */
export function detectLLMFromReferrer(
 ...
 */
export function detectLLMTraffic(
  ...
  if (userAgentResult.isLLM || referrerResult.isLLM) {
    return {
      ...
      confidence:
        userAgentResult.confidence === 'high' || referrerResult.confidence === 'high'
          ? 'high'
 ...
 */
export function getLLMPatternType(result: LLMDetectionResult): string {
  if (!result.isLLM) {
    return 'human';
  }
