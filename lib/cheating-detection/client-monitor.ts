...
export class CheatingMonitor {
  ...
  private monitorTabSwitching() {
    // Log tab switches but throttle them to avoid noise
    // We accumulate switches and send a summary if they happen frequently
    let tabSwitchCount = 0
    let recentSwitchCount = 0
    let lastTabSwitchTime = Date.now()
    let lastLogTime = 0
    const logThrottle = 30000 // Only log tab switches every 30 seconds max
    const tabSwitchWindow = 60000 // 1 minute window for counting total rate

    const handleVisibilityChange = () => {
      const now = Date.now()
      const isHidden = document.hidden
      
      // Reset total counter if enough time has passed
      ...
      // Only log event if throttle time has passed OR if it's highly suspicious (high frequency)
      // This prevents flooding the logs/alerts with rapid tab switching
      const isHighFrequency = recentSwitchCount >= 5;
      
      if (now - lastLogTime >= logThrottle || isHighFrequency) {
        this.logEvent({
          type: 'tab_switch',
          timestamp: now,
          details: {
            ...
            recentSwitches: recentSwitchCount,
            sessionId: this.sessionId,
            suspicious: tabSwitchCount >= 3,
            severity: tabSwitchCount >= 5 ? 'high' : tabSwitchCount >= 3 ? 'medium' : 'low',
          },
        })
        
        lastLogTime = now
        recentSwitchCount = 0 // Reset recent count after logging
      }
    }

    document.addEventListener('visibilitychange', handleVisibilityChange)

    // Also monitor window blur for additional detection, using same throttling
    window.addEventListener('blur', () => {
      // Treat blur similar to visibility change (often they happen together)
      // We'll let the visibility change handler handle the main logic to avoid double logging
      // But if we need specific blur detection, we can add it here with similar throttling
    })
  }
