...
import { Session, Problem } from '@/types/database'
    ...
    const cheatingChannel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          // If it's an LLM attempt, also add to LLM attempts
          if (attempt.attempt_type === 'llm-api-request') {
  ...
  const fetchCheatingAttempts = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Filter LLM attempts
        setLlmAttempts(attempts.filter((a: any) => a.attempt_type === 'llm-api-request'))
      }
    } catch (error) {
                  ...
                  <ListItem>
                    <ListItemText
                      primary={
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
                          <Warning 
                            color={attempt.attempt_type === 'llm-api-request' ? 'error' : 'warning'} 
                            fontSize="small" 
                          />
                          <Typography variant="subtitle2" sx={{ fontWeight: 600 }}>
