...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check IP monitoring for this session (only for problematic events)
    let ipSuspicious = false;
    try {
      const ipCheckResponse = await fetch(
        `${request.nextUrl.origin}/api/monitoring/check-ip`,
        {
          method: 'POST',
          headers: {'Content-Type': 'application/json'},
          body: JSON.stringify({sessionId, clientIp: ipAddress}),
        },
      );
      if (ipCheckResponse.ok) {
        const ipData = await ipCheckResponse.json();
        ipSuspicious = ipData.suspicious || false;
      }
    } catch (err) {
    ...
    if (attemptType === 'llm-api-request') {
      try {
        ...
        const {data: sessionDetails} = await serviceClient
          .from('sessions')
          .select('start_time, end_time, time_limit, session_status, client_ip')
          .eq('id', sessionId)
          .single();
        ...
        if (sessionDetails) {
          ...
          aiRequestDuringActiveSession =
            ...
            now >= sessionStart &&
            now <= sessionEnd &&
            session.client_ip === ipAddress; // Same IP as session
        }
      } catch (err) {
