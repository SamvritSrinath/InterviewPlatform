...
export async function POST(request: NextRequest) {
  try {
    ...
    // Only interviewers/admins can create public sessions or assign interviewers
    if ((isPublic || interviewerId) && !user.is_interviewer && !user.is_admin) {
      return NextResponse.json(
        { error: 'Only interviewers can create public sessions or assign interviewers' },
        { status: 403 }
      )
    }
    ...
    // Log user IP
    const sessionTyped = session as Database['public']['Tables']['sessions']['Row']
    const ipData: Database['public']['Tables']['user_ips']['Insert'] = {
      user_id: user.id,
    ...
    try {
      // TypeScript inference issue with Supabase insert - using type assertion as workaround
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      await (serviceClient.from('user_ips') as any).insert([ipData])
    } catch (err: any) {
